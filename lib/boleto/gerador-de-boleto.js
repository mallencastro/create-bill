...
const BoletoSVG = require('./svg/boleto');
	...
	GeradorDeBoleto.prototype.gerarPDF = function(args) {
		return new Promise((resolve)=> {
			...
			boletos.forEach(function escreveOsDadosDoBoleto(boleto, indice) {
				...
				/* var number = '00190.00009 02729.039004 10011.600177 5 82660000000500';
				var svg = new BoletoSVG(number).toSVG();
				SVGtoPDF(pdf, svg, 30, args.ajusteY + 180, {
					width: 340,
				}); */
				...
				boleto.getLocaisDePagamento().forEach(function(localDePagamento, indice) {
					...
					pdf.font('normal')
						.fontSize(args.tamanhoDaFonteDoTitulo)
						.text(localDePagamento, args.ajusteX + margemDoSegundoBloco, args.ajusteY + (terceiraLinha + 6 - args.tamanhoDaFonte + (indice * args.tamanhoDaFonte)), {
							lineBreak: false,
							width: 400,
							align: 'left'
						});
